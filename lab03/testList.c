	...
	while (!done && getCommand(cmd)) {
	  ...
	  switch (cmd[0]) {
		...
		case 'i':
			...
			printf("Enter line to insert at front: \n");
			fgets(new,MAX,stdin);
			new[strlen(new)-1] = '\0';
		...
		case 'a':
			...
			printf("Enter line to insert after: \n");
			fgets(new,MAX,stdin);
			new[strlen(new)-1] = '\0';
...
void showfirst(DLList lines){
     printf("%s",DLListfirst(lines));
     printf("\n");
}
// giveHelp()
// show help message
